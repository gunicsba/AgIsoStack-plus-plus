//================================================================================================
/// @file vt_retry_tests.cpp
///
/// @brief Unit tests for the VT server retry mechanism
...
// Test the retry tracking functionality in VirtualTerminalServerManagedWorkingSet
TEST(VTRetryTests, TestRetryTracking)
{
	...
	// Test incrementing beyond max retries
	for (int i = 0; i < 10; i++)
	{
		workingSet->increment_retry_count();
	}
...
// Test the retry mechanism in VirtualTerminalServer
TEST(VTRetryTests, TestVTServerRetryMechanism)
{
	// This test would require more complex mocking of the network manager and ETP sessions
	// For now, we'll just verify that the method exists and can be called
	EXPECT_TRUE(true); // Placeholder
}

// Test the ETP session restart mechanism
TEST(VTRetryTests, TestSessionRestartMechanism)
{
	// Create a mock control function
	auto mockControlFunction = std::make_shared<isobus::ControlFunction>(isobus::NAME(0), 0x12, 0);
...
TEST(VTRetryTests, TestMaxRetryCount)
{
